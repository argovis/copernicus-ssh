...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // file opening
    //let file = netcdf::open("data/xx_ssh_mean_1993.nc")?;
    ...
    for latidx in 175..176 {
        let mut meanslabatch = Vec::new();
        for _lonidx in 0..1440 {
            meanslabatch.push(Vec::new());
        }

        for _f in 0..30 { //batchfiles.len() { // ie for every year
            let file = netcdf::open(batchfiles[0])?; //netcdf::open(batchfiles[_f])?; 
